...
func main() {
	...
	ps1 := genPointSeq(pl, []int{2, 3, 4, 5, 6})
	ps2 := genPointSeq(pl, []int{2, 1, 8, 7, 6})

	pl = genPointList(500, 500, 400, 400)
	ps3 := genPointSeq(pl, []int{2, 3, 4, 5, 6})
	ps4 := genPointSeq(pl, []int{2, 1, 8, 7, 6})

	incr := 8
	segments := [][]point{ps1, ps2, ps3, ps4}
	var vertices [][]point
	...
	/*
		incr := 8
		v1 := []point{{200, 100}, {300, 100}, {300, 300}, {200, 300}, {300, 300}}
		v2 := []point{{200, 100}, {100, 100}, {100, 300}, {200, 300}, {100, 300}}

		segments := [][]point{v1, v2}
		var vertices [][]point
		for _,s := range segments {
			v := calcWaypoints(s, incr)
			vertices = append(vertices, v)
